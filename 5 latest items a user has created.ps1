# Lists 5 latest items a user has created in SharePoint Online list and exports them to "c:\users\public\FiveItems.csv"
...
Type='User'>"+$UserDisplayName+"</Value></Eq></Where></Query><RowLimit>5</RowLimit></View>";
  ...
  foreach($item in $items)
  {
        ...
        $Object | add-member LastModified $item.Fieldvaluesastext.FieldValues["Created"]
        Export-Csv -InputObject $Object -Path "c:\users\public\FiveItems.csv" -Append
  }
